...
% Set up Bnew
%
B     = B(:);%convert B into column vector.p;
Bnew  = [B(1:r-1);s;B(r+1:mt-1)];%Bnew is a column vector.
%The corresponding B(r) should be removed.The r-th column should be
%replaced by the s-th column.
%mt-1 because the row number of A matrix is mt-1
% Setup Tnew
%
Tnew         = zeros(mt,nt);
if (t(r) == 0)
